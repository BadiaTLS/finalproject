    ...
    if request.method == 'POST':
        ...
        if choice == 'take':
            ...
            if booking.exists():
                ...
                if booking.latest('id').available == 0:
                    messages.warning(request, 'The vacancy is already full.')
                    return redirect('dining_hall')
            else:
                vacancy = menu.limit - 1
            booking = table_booking_dininghall.objects.create(students_nim=student, available=vacancy, time_booked=time_suggested, menu=menu)
